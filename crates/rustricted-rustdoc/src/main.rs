//! `rustricted-rustdoc` — `RUSTDOC` (and `RUSTDOC_WRAPPER`) shim that runs
//! Rustricted's lowering pass before invoking the real `rustdoc`.
//!
//! ## Why this exists
//!
...
//! that uses Rustricted syntax extensions (named args, pipe) fails
...
//!
//! The lowering itself is the shared `rustricted_rustc::prepare_strict_input`
