...
//! `STD_SIGNATURES` (in `rustricted-lower`) used to be generated by
//! parsing this file directly with `syn`, which would have broken the
//! moment the file used any Rustricted-specific syntax. The current
//! design generates a checked-in manifest at
//! `crates/rustricted-std/std-signatures.txt` via
//! `cargo xtask gen-std-signatures` (which lowers the source first, then
//! parses), and `rustricted-lower/build.rs` reads that manifest. CI
