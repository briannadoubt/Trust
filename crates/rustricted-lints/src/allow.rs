...
//!
//! Non-rustricted allows (`#[allow(dead_code)]`, `#[allow(clippy::xxx)]`)
...
impl AllowMap {
    ...
    /// `#[allow(rustricted::<rule>)]` attribute.
...
/// - `None` if this attribute isn't an `#[allow(rustricted::...)]` form
...
fn parse_rustricted_allow(attr: &syn::Attribute) -> Option<ParsedAllow> {
    ...
    let parse_result = attr.parse_nested_meta(|m| {
        ...
        // Two-segment path `rustricted::Rxxxx`.
        if m.path.segments.len() == 2 && m.path.segments[0].ident == "rustricted" {
            saw_rustricted = true;
        ...
        // Non-rustricted path (e.g. `dead_code`, `clippy::xxx`) — accept and
...
fn process_attrs(
    ...
    for attr in attrs {
        let Some(parsed) = parse_rustricted_allow(attr) else {
            continue;
        };
        ...
        for (code, range) in &parsed.unknown_codes {
            let diag = Diagnostic::error(
                ...
                format!("`#[allow(rustricted::{code})]` references an unknown rule code"),
                range.clone(),
            )
        ...
        if !reason_ok {
            let diag = Diagnostic::error(
                ...
                "`#[allow(rustricted::...)]` missing `reason = \"...\"` justification",
...
/// Collect every `#[allow(rustricted::...)]` scope in the file. Also emits
