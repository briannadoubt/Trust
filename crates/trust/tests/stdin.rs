//! Integration tests for stdin (`-`) input support in the `trust` CLI.
//!
//! These shell out to the binary cargo built for this test target via the
//! `CARGO_BIN_EXE_trust` env var, so no extra dev-dependency is needed.
...
fn run_with_stdin(args: &[&str], stdin_input: &str) -> (bool, String, String) {
    let mut child = Command::new(trust_bin())
        .args(args)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawning trust");
    ...
    let output = child.wait_with_output().expect("waiting on trust");
    (
