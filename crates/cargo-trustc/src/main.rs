...
fn run_cargo(args: &[String]) -> Result<i32> {
    ...
    let auto_index = auto_signature_index(&args);
    if let Some(path) = &auto_index {
...
fn auto_signature_index(args: &[String]) -> Option<PathBuf> {
    ...
    let text = trust_lower::sig_index::render_manifest(
        &merged,
        "# @generated by `cargo trustc` (RT-114): workspace public-fn signature index.",
    );
    let out = env::temp_dir().join(format!("trust-sigindex-{}.txt", std::process::id()));
