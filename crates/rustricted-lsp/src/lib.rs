//! Language Server Protocol implementation for Rustricted.
//!
//! Runs the same lower + lint pipeline as the CLI (`crates/rustricted/src/main.rs`)
...
use rustricted_diag::{Diagnostic as RtDiag, Severity};
use rustricted_lower::named_args::CalleeRegistry;
...
impl LanguageServer for Backend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "rustricted-lsp initialized")
            .await;
    }
...
pub fn compute_diagnostics(source: &str) -> Vec<Diagnostic> {
    ...
    let lower_out = match rustricted_lower::lower(source) {
        Ok(out) => out,
        Err(e) => {
            ...
            let diag = Diagnostic {
                ...
                source: Some("rustricted".to_string()),
