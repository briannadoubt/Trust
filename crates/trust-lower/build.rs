...
//!
//! - RT-32 parsed `trust-std/src/lib.rs` directly with `syn` at build
//!   time. That worked as long as the std-shim source stayed plain Rust.
//! - RT-31 dogfooding discovered the circular trap: if `trust-std`
...
//!   manifest is auditable, the build is decoupled from `trust-std`'s
...
fn main() {
    ...
    out.push_str("// @generated by trust-lower/build.rs from\n");
    out.push_str("// crates/trust-std/std-signatures.txt — do not edit.\n");
