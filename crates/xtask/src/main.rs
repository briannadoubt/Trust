...
//!   `crates/rustricted-std/std-signatures.txt` from
//!   `crates/rustricted-std/src/lib.rs` (lowered, then parsed with `syn`).
...
/// Regenerate `crates/rustricted-std/std-signatures.txt` from
/// `crates/rustricted-std/src/lib.rs`.
///
/// `rustricted-std` may itself be `#![strict]`-marked, so the source is
/// first passed through `rustricted_lower::lower()` to desugar any
/// Rustricted-specific syntax (strict markers, named args, pipes), then
...
fn gen_std_signatures(check_only: bool) -> Result<()> {
    let root = repo_root()?;
    let lib_path = root.join("crates/rustricted-std/src/lib.rs");
    let manifest_path = root.join("crates/rustricted-std/std-signatures.txt");
    ...
    let lowered = rustricted_lower::lower(source: &raw)
...
# `crates/rustricted-std/src/lib.rs`. Do not edit by hand — run the xtask
# subcommand after modifying the std-shim source. CI verifies this file is
# fresh via `cargo xtask gen-std-signatures --check`.
#
# Format: one line per public `fn` in `rustricted-std`. Each line is
#
...
#
# This file is the source of truth that `rustricted-lower/build.rs` reads
# to seed `STD_SIGNATURES`. The previous design parsed `lib.rs` with
# `syn` at build time, but that meant `rustricted-std` could never itself
# be `#![strict]`-marked (syn rejects named-arg syntax). The checked-in
# manifest removes that coupling — `lib.rs` can use any Rustricted feature
