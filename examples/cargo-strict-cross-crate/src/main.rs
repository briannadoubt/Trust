...
// crate (`rustricted-std`). Without cross-crate seeding the call site
// `rustricted_std::fs::write_text(path: ..., contents: ...)` is passed
...
fn main() {
    ...
    let path: PathBuf = tmp_dir.join("rustricted-cross-crate.txt");

    // Args intentionally supplied in reverse order to exercise the
    // reorder path.
    rustricted_std::fs::write_text(contents: "ok\n", path: &path)
        .expect("write_text");

    let read = rustricted_std::fs::read_to_string(path: &path)
        .expect("read_to_string");

    let _ = rustricted_std::fs::remove_file(path: &path);
