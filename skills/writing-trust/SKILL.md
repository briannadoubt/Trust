...
Trust is a strict dialect of Rust: 20 hard lints (R-codes) plus three syntax extensions, lowered to plain Rust before rustc sees it. Build with `cargo trustc build` (or `trust build` for single files) — **never plain `cargo`**: stock rustc rejects the dialect syntax and skips the lints.

## Project setup

- **Greenfield:** `trust new <name>` scaffolds a strict project (Cargo.toml with the metadata key, named-arg hello-world, .gitignore, README). Build it with `cargo trustc build`.
...
**Important:** this syntax only compiles via `cargo trustc` / `trust build`. Plain `cargo build` failing on `f(width: 1920, ...)` is *expected behavior*, not a bug — do not "fix" it by deleting the names; switch the build command instead.

## The iteration loop

1. `cargo trustc build --message-format json` — Trust diagnostics arrive on **stderr** as one JSON document per failing file (same shape as `trust check --format json`):
...
- Single-file lint without cargo: `trust check foo.rs --format json`. Note `cargo trustc check` resolves to whole-crate `cargo check` instead.

Command quick reference:

```sh
trust new <name>                            # scaffold a strict project
cargo trustc build|run|test                  # cargo lifecycle through the shims
cargo trustc build --message-format json     # machine-readable diagnostics on stderr
...
<!-- Generated from `cargo run -q -p trust-lang -- explain --format json` (catalogue v0.1)
...
- `#[allow(trust::R0xxx, reason = "…")]` works ONLY in code built exclusively via `cargo trustc` — stock rustc rejects tool-scoped allows, so published/stock-buildable crates must fix the violation, not suppress it.
...
- `// safety:` (R0005) and `// reason:` (R0006) comment contracts go in the contiguous comment block **directly above** the site — a blank line breaks the association:
  ...
  ```
- `cargo trustc build --message-format json` is Trust's flag, not cargo's — cargo's own `--message-format` takes different values; the wrapper strips the flag and sets `TRUST_MESSAGE_FORMAT=json`. Setting that env var directly is equivalent.
