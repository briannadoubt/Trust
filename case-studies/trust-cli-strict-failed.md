...
target/debug/trust-rustc) cargo build` succeeds on a file using
...
previous (small) dogfood pass on `trust-syntax` did not surface.
...
work, added `trust_attrs::strict!{}` to the top, ran
`trust check`, and saw 8+ R0042 violations on legitimate multi-arg
...
error: expected one of `)`, `,`, `.`, `?`, `}`, or an operator,
       found `:`
   --> crates/trust/src/main.rs:96:36
    |
...
`#![strict]` and `trust_attrs::strict!{}` only activate the
toolchain's **lints**. The toolchain's **syntax extensions** (named args,
pipe operator, `effect` keyword) require the Trust **lowering
...
(`trust check`, `trust build`). `cargo build` invokes rustc
...
1. A cargo-level integration (`RUSTC_WRAPPER` shimming `trust lower`
   in front of rustc). Standard pattern, real engineering work.
2. The `trust_attrs::strict!{}` macro becomes a full preprocessor
...
## What `trust-attrs` *does* still buy
...
can opt in with `trust_attrs::strict!{}` and get those lints. This is
the slice that worked for `trust-syntax` (60 LOC, all arity-1
...
Calling the cargo-mode subset "Trust" without that caveat oversells
...
`trust_attrs::lints!{}`) to signal it's the lint-only subset.

## Recommended follow-ups

- **High value, medium effort**: build the cargo `RUSTC_WRAPPER`
  integration so `cargo build` can invoke Trust lowering before
