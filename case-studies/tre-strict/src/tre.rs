...
pub fn run(option: RunOptions) {
    let filter = if option.directories_only {
        path_finders::EntryFilter::DirectoriesOnly
    } else {
        path_finders::EntryFilter::All
    };
    let max_depth = option.max_depth.unwrap_or(std::usize::MAX);
    let paths: Vec<(String, FileType)> = match option.mode {
        Mode::FollowGitIgnore => {
            path_finders::find_non_git_ignored_paths(root: &option.root, filter: filter, max_depth: max_depth)
        }
        Mode::ExcludeHiddenFiles => {
            path_finders::find_non_hidden_paths(root: &option.root, filter: filter, max_depth: max_depth)
        }
        Mode::ShowAllFiles => {
            path_finders::find_all_paths(root: &option.root, filter: filter, max_depth: max_depth)
        }
    };
    ...
    if option.output_json {
        println!("{}", json_formatting::format_paths(root_path: &option.root, children: paths));
    } else {
        let path_style = if option.portable_aliases {
            diagram_formatting::PathStyle::Absolute
        } else {
            diagram_formatting::PathStyle::Relative
        };
        let format_result =
            diagram_formatting::format_paths(root_path: &option.root, children: paths, path_style: path_style);
        ...
        if let Some(editor) = option.editor {
            output::print_entries(entries: &format_result, alias_mode: output::AliasMode::On, lscolors: coloring);
            let editor = if cfg!(windows) {
                editor.unwrap_or_else(|| "".to_string())
            } else {
                editor.unwrap_or_else(|| "$EDITOR".to_string())
            };
            output::create_edit_aliases(editor: &editor, entries: &format_result);
        } else {
            output::print_entries(entries: &format_result, alias_mode: output::AliasMode::Off, lscolors: coloring);
        }
    }
}
