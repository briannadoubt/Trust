...
/// R0012: replaced `create_alias: bool` with a named enum so swapped or
/// misread booleans at call sites can't silently flip alias generation.
...
pub fn print_entries(entries: &[FormattedEntry], alias_mode: AliasMode, lscolors: Option<&LsColors>) {
    let create_alias = matches!(alias_mode, AliasMode::On);
    let number_color = lscolors.map(|_| ColorSpec::new().set_fg(Some(Color::Red)).to_owned());
    for (index, entry) in entries.iter().enumerate() {
        if create_alias {
            print!("{}[", entry.prefix);

            color_print(text: index, color: number_color.as_ref());
            print!("] ");
        } else {
            print!("{}", entry.prefix);
        }
        ...
        color_print(text: &entry.name, color: spec.as_ref());
        println!()
    }
}
