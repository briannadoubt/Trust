...
impl File {
    ...
    fn add_child(&mut self, name: &str, id: usize) {
        if let TypeSpecficData::Directory(children) = &mut self.data {
            ...
            children.insert(key: name.to_string(), value: id);
        }
    }
...
impl FileTree {
    pub fn new(root_path: &str, children: Vec<(String, FileType)>) -> Option<FileTree> {
        ...
        for (path, meta) in children {
            ...
            // reason: data_option is Some for every variant of `meta` above
            // except a non-resolvable symlink target; in that case we want to
            // skip the entry rather than crash, so unwrap_or_continue.
            ...
            // Was: `if ancestor.is_none() { continue; } let path_name = ancestor.unwrap();`
            // — collapsed to a single match to remove R0001 unwrap.
            let path_name = match ancestry.pop().map(|x| to_string(&x)) {
                Some(n) => n,
                None => continue,
            };
            ...
            for ancestor_name in ancestry {
                ...
                // R0014: bare-index `slab[id]` replaced with `.get(id)` + expect.
                // reason: ids in `current_acestor_id` come from slab.vacant_entry().key()
                // and are valid for the lifetime of the slab in this builder loop.
                let ancestor_node = slab
                    .get(current_acestor_id)
                    .expect("FileTree builder invariant: current_acestor_id is a live slab entry");
                if let Some(child_key) = ancestor_node.child_key(&display_name) {
                    ...
                    slab.get_mut(current_acestor_id)
                        .expect("FileTree builder invariant: current_acestor_id is a live slab entry")
                        .add_child(name: &display_name, id: new_id);
            ...
            slab.get_mut(current_acestor_id)
                .expect("FileTree builder invariant: current_acestor_id is a live slab entry")
                .add_child(name: &path_name, id: new_id);
        }
    ...
    pub fn get(&self, id: usize) -> &File {
        // reason: `id` must be a key obtained from this FileTree's own storage —
        // out-of-range ids indicate a logic bug in the caller. R0014: replaced
        // `&self.storage[id]` with `.get(id).expect(...)`.
        self.storage
            .get(id)
            .expect("FileTree::get called with id not in storage")
    }
